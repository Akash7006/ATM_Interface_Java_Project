...
class Account {
    ...
    void createAcc() {
        ...
        int length_pin = 0;
        do {
            System.out.println("Enter secret 4 digity pin?");
            pin = in.nextInt();
            length_pin = String.valueOf(pin).length();
        } while (length_pin != 4);
        System.out.println("Enter initial deposit amount?");
        balance = in.nextDouble();
        System.out.println("Congratulations Your Amount Successfully Created!!\n");
        System.out.println("Account Details-\n" + "Account Number- " + a_no + "\nAccount Holder Name- " + acc_holder_name + "\nBalance-" + balance + "\nThank You");
        String fileName = acc_number + ".text";
        ...
        try {
            ...
            writer.write("USER TO-r: " + unique_id + "\n");
...
class ATM {
    void withdraw(Account acc) {
        ...
        if (amount % 100 == 0) {
            if (acc.balance >= amount) {
                acc.balance -= amount;
                System.out.println("Your  Transaction id Processing\n");
                try {
    ...
    void diposit_by_transfer(Account acc, double amount){
        acc.balance += amount;
        try {
            String fileName = acc.a_no + ".text";
            FileWriter fileWriter = new FileWriter(fileName, true);
            BufferedWriter bufferedWriter = new BufferedWriter(fileWriter);
            bufferedWriter.write("Diposit : " + amount + "\n");
            bufferedWriter.write("Date : " + new Date() + "\n");
            bufferedWriter.write("Account Number: " + acc.a_no + "\n");
            bufferedWriter.write("Remaining Balance: " + acc.balance + "\n");
            bufferedWriter.close();
    ...
    void diposit(Account acc){
        Scanner in = new Scanner(System.in);
        System.out.print("\033[H\033[2J");
        System.out.flush();
        System.out.println("Money Deposit Mode\n");
        System.out.println("Enter Amount  You Went To Diposit?");
        double amount = in.nextDouble();
        acc.balance += amount;
        System.out.println("\033[H\033[2J");
        System.out.flush();
    ...
    void Transfer(Account acc1, Account acc2, double amount){
        if (acc1.balance >= amount){
            acc1.balance -= amount;
            ATM a = new ATM();
            a.diposit_by_transfer(acc2, amount);
            try {
                String fileName = acc1.a_no + ".text";
                FileWriter fileWriter = new FileWriter(fileName, true);
                BufferedWriter bufferedWriter = new BufferedWriter(fileWriter);
    ...
    void display_details(Account acc){
        ...
        String file_name = String.valueOf(acc.a_no) + ".text";
        ...
        System.out.println("Screen Will Automatically Timeout After 30 Second . . . ");
        try {
            Thread.sleep(30000);
        } catch (InterruptedException e) {
            e.printStackTrace();
...
class run_atm {
    int account_search_by_uniqui_id(Account[] ac, String unique_id) {
        for (int i = 0; i < 5; i++) {
    ...
    int account_search_by_uniqui_id(Account[] ac, int account_number) {
        for (int i = 0; i < 5; i++) {
    ...
    void demo(Account[] ac) {
        Scanner in = new Scanner(System.in);
        System.out.print("\n\nWelcome To ATM\n");
        System.out.print("\nEnter Youe Card/Unique ID");
        String unique_id = in.nextLine();
        int i = account_search_by_uniqui_id(ac, unique_id);
        if (i == -1) {
...
class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 5; i++) {
            System.out.print("\033[H\033[2J");
            System.out.flush();
            System.out.print("Creating Account Mode-\n");
            ac[i] = new Account();
            ac[i].createAcc();
            System.out.print("\033[H\033[2J");
            System.out.flush();
